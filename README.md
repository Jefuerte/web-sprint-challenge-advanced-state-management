...
context api shares data among multiple components without having to drill props.

2. In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?
actions carry payloads from application to store.
reducers are just functions that take an action and previous state then returns a new state.
store holds the apps state. Storeredux can only have one store.

3. What does `redux-thunk` allow us to do? How does it change our `action-creators`?
redux-thunk is middleware that allows reuse of functions.
action-creator is changed because it uses a dispatch.

4. What is your favorite state management system you've learned and this sprint? Please explain why!

my favorite is context api because its easier and it can access all the data provided by a provider. 
